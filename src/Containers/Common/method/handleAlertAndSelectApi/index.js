export const handleAlertAndSelectApi = (
  responseData,
  actionSelect,
  dispatch,
  argomentAction
) => {
  if (responseData.error === "INVALID INPUT token: Not Authorized") {
    dispatch({
      type: "ALERT",
      payload: {
        status: true,
    ...
    responseData.response.is_successful
  ) {
    dispatch({
      type: "ALERT",
      payload: {
        status: true,
        textAlert: "درخواست شما با موفقیت انجام شد.",
        typeAlert: "success",
      },
    });
  ...
  //     }, 1000);
  // } else if (responseData.status === 200 && responseData.response.error_description === "PERMISSION DENIED") {
  //     dispatch({ type: "ALERT", payload: { status: true, textAlert: `شما دسترسی به انجام این عملیات را ندارید.`, typeAlert: "error" } })

  // }
  // else if (responseData.status === 200 && !responseData.response.is_successful) {
  //     dispatch({ type: "ALERT", payload: { status: true, textAlert: `خطایی رخ داد(${responseData.response.error_description})`, typeAlert: "error" } })
...
export const handleNoAnswarApi = (dispatch) => {
  dispatch({
    type: "ALERT",
    payload: {
      status: true,
...
export const handleAlertMethodSelect = (responseData, dispatch) => {
  if (responseData.error === "INVALID INPUT token: Not Authorized") {
    dispatch({
      type: "ALERT",
      payload: {
...
const handelError = (key) => {
  switch (key) {
    ...
    case 104101:
      return "مدت زمان ویرایش به پایان رسیده";
    case 101101:
    ...
    case 108305:
      return "کالای غیر فیزیکی نمی تواند کد رهگیری پستی داشته باشد.";
    case 108304:
      return "جایزه ای که نهایی نشده نمی تواند کد رهگیری پستی داشته باشد.";
    case 108303:
      return " کد رهگیری پستی تکراری می باشد.";
    case 108302:
      return "کد کالا جایزه تکراری می باشد.";
    case 115301:
      return "کاربر قبلاً یک درخواست مشابه ارسال کرده است.";
    case 113201:
      return "در حال حاضر امکان ارسال ایمیل وجود ندارد.";
    case 113101:
      return "فقط قادر به پاسخ دادن به تیکت های پاسخ داده نشده هستید.";
    case 120102:
      return "این کد قبلا استفاده شده است و امکان ویرایش آن وجود ندارد";
    default:
