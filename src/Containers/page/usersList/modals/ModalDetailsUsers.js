...
export default function ModalDetailsUsers({ data }) {
  ...
  return (
    <div className={classes.modalDetail}>
      ...
      <div style={{ width: "25%" }}>
        <p>ایمیل: {data.body.email === "null" ? "-" : data.body.email}</p>
      </div>

      {
        data.body.introducing_branch_id !== "null" && (
          <>
            <div style={{ width: "25%", margin: "0 10px" }}>
              <p>نام شعبه: {branchName(data.body.branch_name)}</p>
            </div>
            <div style={{ width: "25%" }}>
              <p>کد شعبه: {handleNull(data.body.introducing_branch_id)}</p>
            </div>
            <div style={{ width: "20%" }}>
              <p>استان شعبه: {handleNull(data.body.branch_city)}</p>
            </div>
            <div style={{ width: "20%" }}>
              <p>شهر شعبه: {handleNull(data.body.branch_province)}</p>
            </div>
          </>
        )
      }
