...
export default function Index({ data, index, setNewButton, handel_Remove_forum, handel_update_forum, handel_enable_forum, name  , handel_insert_forum , handel_update_name_formus }) {
    ...
    const handelDelete = () => {
        setState(prev => ({
            ...prev,
            subgroup_name: prev.subgroup_name.filter((items, ind) => ind !== indexDate)
        }))
        setOpenAlert(false)
    }
    ...
    const handel_ADD_Items = (name) => {
        setState(prev => ([
            {
                id:'',
                body :  {name: name, author_id: "", subgroup_name: "", is_visible: "" , insert:true},
            },
            ...prev,
            ]))
    }
    ...
    return (
        <div className={classes['card']}>
            ...
            <Box className={classes['btns']}>
                ...
                <button className={`${classes['btnsAdd']} btnsBlue`} onClick={() => handel_ADD_Items(name)}>افزودن</button>
