...
export default function Index({
    ...
    const [state, setState] = useState({ name: "", id: "", is_visible: "", subgroup_name: "" })
    ...
    const handelSumbit = () => {
        ...
        if (flag === 'remove') {
            if (state.is_visible === 'TRUE')
                handel_Remove_forum(state)

            if (state.is_visible === 'FALSE')
                handel_enable_forum(state)
    ...
    return (
        <>
            <div className={classes['card']}>
                {/* {
                flagIconDelete && (
                    <DeleteIcon className={classes['iconDelete']} onClick={() => showAletDelete(index)} />
                )
            } */} 
                ...
                <TextField
                    ...
                    size="small"
                    value={state.name}
                    onChange={(event) => handelChangeSubgroup_name(event.target.value , 'name')}
                />
                ...
                <TextField
                    ...
                    onChange={(event) => handelChangeSubgroup_name(event.target.value , 'subgroup_name')}
                />

                {
                    flagIconSwitch && !state.insert  &&(
                        <Switch inputProps={{ 'aria-label': 'primary checkbox' }} color="primary" checked={state.is_visible === 'FALSE' ? false : state.is_visible === 'TRUE' ? true : ''} onClick={() => { handelShowAlertDialogSlide('remove') }} />
                    )
                }
