...
export default function Index({ handleChangeText, state, handleSubmit, handleExit }) {
	...
	return (
		<div className={classes['ModalAdd']}>
			...
			<div style={{ display: 'flex', justifyContent: 'space-around', marginBottom: '60px', width: '90%' }}>
				<Box width={250}>
					<DataPicker label={'تاریخ عرضه'}>
						{(date) => {
							handleChangeText(date, 'dateIpo');
						}}
					</DataPicker>
				</Box>
				<Box width={250}>
					<DataPicker label={'تاریخ شروع ثبت نام'}>
						{(date) => {
							handleChangeText(date, 'dataStart');
						}}
					</DataPicker>
				</Box>
				<DataPicker label={'تاریخ پایان ثبت نام'}>
					{(date) => {
						handleChangeText(date, 'dateEnd');
					}}
				</DataPicker>
			</div>
