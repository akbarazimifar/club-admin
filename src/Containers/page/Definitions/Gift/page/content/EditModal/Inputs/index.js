    ...
    <div className={Styles["inputs"]}>
      ...
      <Box width="23%" mr={1}>
        <TextField
          ...
          margin="dense"
          onChange={(event) =>
            handelCHnage(event.target.value, "gift_category")
          }
        />
      </Box>
      ...
      <Box width="23%">
        <DatePickerEdit label="تاریخ انقضاء" value={state.expiration_time}>
          {(data) =>
            handelCHnage(
              `${data ? `${dateMiladi(data)} 23:59:00.000000` : ""}`,
              "expiration_time"
            )
          }
        </DatePickerEdit>
          ...
          <FormControlLabel
            control={
              <Checkbox
                ...
                onChange={(e) =>
                  handelCHnage(!state.is_physical, "is_physical")
                }
      ...
      <Box width={"100%"} height={350} mt={4}>
        {state.detailed_description && (
          <TextEditorQuill answerDataEdit={state.detailed_description}>
            {(data) =>
              handelCHnage(
                data === undefined ? "" : data,
