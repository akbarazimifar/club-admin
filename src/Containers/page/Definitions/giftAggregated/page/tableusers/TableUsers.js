...
const tableHead = [
  ...
  { id: 2, label: "نام و نام خانوادگی", title: "نام و نام خانوادگی", active: false },
  { id: 4, label: "کد ملی", title: "کد ملی: ", active: false },
  { id: 4, label: "کد تفصیلی", title: "is_removed", active: false },
  { id: 3, label: "وضعیت", title: "to_date_time", active: false },
  { id: 3, label: "مجموع امتیاز", title: "to_date_time", active: false },
];
...
const TableUsers = ({
  ...
  const resultAlldata = () => {
    ...
    .forEach((item => {
      ...
      let condition = "تایید شده"

      let flag = item.body["وضعیت"] === condition

      if (flag)
        Positive += item.body['مجموع امتیاز']
      if (!flag)
        negative += item.body['مجموع امتیاز']
    }))
    setTotal(
      {
              ...
              ? data.data
                ...
                .map((row, ind) => (
                  <TableRow key={ind}>
                    ...
                    <TableCell className="colorInherit" align="center">
                      {handleNull(row.body["نام و نام خانوادگی"])}
                    </TableCell>
                    <TableCell className="colorInherit" align="center">
                      {handleNull(row.body['کد ملی'])}
                    </TableCell>
                    <TableCell className="colorInherit" align="center">
                      {handleNull(row.body['کد تفصیلی'])}
                    </TableCell>
                    <TableCell className="colorInherit" align="center">
                      {handleNull(row.body['وضعیت'])}
                    </TableCell>
                    <TableCell className="colorInherit" align="center">
                      {handleNull(row.body['مجموع امتیاز'])}
                    </TableCell>
                  </TableRow>
                ))
              : null}
