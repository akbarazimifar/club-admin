...
const TableUsers = ({
  ...
  const tableHeadStart = [
    ...
    { id: 3, label: " تاریخ شروع مغایرت", title: "conflict_from_date", active: false },
    { id: 3, label: "تاریخ پایان مغایرت", title: "conflict_to_date", active: false },
    { id: 3, label: "اختلاف امتیاز  تخفیف پله ای روزانه", title: "diff_daily_turnover_score", active: false },
    { id: 3, label: "اختلاف امتیاز معرف", title: "diff_introducer_turnover_score", active: false },
    { id: 3, label: "اختلاف امتیاز معاملات آتی", title: "diff_monthly_future_turnover_score", active: false },
  ];
              ...
              ? data.data.map((row, ind) => {
                return (
                  <TableRow key={ind}>
                    ...
                    <TableCell className="colorInherit" align="center">
                      {handleNumber(row.body.member_national_id)}
                    </TableCell>
                    <TableCell className="colorInherit" align="center">
                      {handleNull((row.body.conflict_from_date && row.body.conflict_from_date !== 'null') ? dateMiladiToShamsi(row.body.conflict_from_date.split(' ')[0]) : '')}
                    </TableCell>
                    <TableCell className="colorInherit" align="center">
                      {handleNull((row.body.conflict_to_date && row.body.conflict_to_date !== 'null') ? dateMiladiToShamsi(row.body.conflict_to_date.split(' ')[0]) : '')}
                    </TableCell>
                    <TableCell className="colorInherit" align="center">
                      {handleNumber(row.body.diff_daily_turnover_score)}
                    </TableCell>
                    <TableCell className="colorInherit" align="center">
                      {handleNumber(row.body.diff_introducer_turnover_score)}
                    </TableCell>
                    <TableCell className="colorInherit" align="center">
                      {handleNumber(row.body.diff_monthly_future_turnover_score)}
                    </TableCell>
                  </TableRow>
                )
              })
