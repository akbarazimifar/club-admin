...
import { dateMiladi, dateMiladiToShamsi } from "./../../../../../../Common/method/date";
...
String.prototype.allReplace = function (obj) {  //eslint-disable-line no-extend-native
    var retStr = this;
    for (var x in obj) {
...
export default function HorizontalNonLinearStepper({ disable, data, submitUpdateCompetition }) {
    ...
    useEffect(() => {
        setValueInsert({
            competition_title: data.body.competition_title,
            required_bonus: data.body.required_bonus,
            reward_bonus: data.body.reward_bonus,
            start_date: dateMiladiToShamsi(data.body.start_date),
            participation_deadline: dateMiladiToShamsi(data.body.participation_deadline),
            question: data.body.question,
            correct_answer_description: data.body.correct_answer_description,
            updatable: data.body.updatable,
            time_start: moment(new Date(data.body.start_date.allReplace({ " ": "T", "/": "-" }))),
        })
    ...
    const handleComplete = () => {
        if (completedSteps() === totalSteps() - 1) {
            ...
            let result = {
                ...
                required_bonus: valueInsert.required_bonus.toString(),
                reward_bonus: valueInsert.reward_bonus.toString(),
            ...
            submitUpdateCompetition(result)
            disable(false)
            return
        }
